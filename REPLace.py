...
class uploader:
    ...
    def __init__(self):
        ...
        self.port = port
        if not self.port:
            self.port = '/dev/cu.wchusbserialfa130'
        ...
        # notify
        print()
        print('\033[91m' + 'REPL-Ace Uploader - Copyright (c) 2017 Clayton Darwin (class lad)')
    ...
    def upload(self):
        ...
        # end message
        print('STARTING UPLOAD...' + '\033[0m')

        # create connection and import OS
        if not smash_only:
            self.connection = serial.Serial(port=self.port,baudrate=self.baudrate,timeout=self.timeout)
            self.connection.flush()
            self.connection.write([3,3]) # clear = ctrl-c (ETX ASCII character)
    ...
    def send(self,line=''):
        # ord(x) returns unicode character for x
        # converts each character in line to unicode and sends
        # to ESP to run as REPL (eg. 'import os')
        self.connection.write([ord(x) for x in line+'\r'])
        self.recv()       

    # Read back anything that the ESP might send
    def recv(self,done=False):
        ...
        while 1:
            ...
            if not data:
                ...
                self.rbuffer += data.decode(encoding='utf-8',errors='ignore')
